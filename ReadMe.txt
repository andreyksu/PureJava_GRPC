Информация по запуску проекта:
	...
	2. Если IDEA - то после сборки, нужно выполнить 'Reload All Maven Projects':
...
Рабочий пример генерации.
  exe. файл protoc-gen-grpc-java... скачивается отдельно.
C:\utils\protoc-26.0-win64\bin\protoc.exe --plugin=protoc-gen-grpc-java=C:\utils\protoc-gen-grpc-java-1.58.0-windows-x86_64.exe  --java_out=src/main/java --grpc-java_out=src/main/java -I=src/main/proto src/main/proto/fullproto.prot
Нужно смотреть версии генератора и в maven - должны соответствовать (иначе проблемы в компиляции/среде разработки).
//-----------------------------------------------------------
